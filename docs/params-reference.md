...
| `currentStrength` | `1.0` | 0–2 | **D2: generation param, default-on** — ocean-current intensity. A fixed-pass relaxation (`utils/currents.ts`) yields a wind-driven, Coriolis-deflected, continent-blocked velocity field + SST anomaly that moderates coastal temperature (Gulf-Stream warming / cold-current cooling) and boosts warm-current evaporation into the 8-pass moisture transport. **0 = byte-identical escape hatch** (stage short-circuits). Live in paramLiveness; **regenerates** on change. (Nuance: the warm-current evaporation boost raises the ocean moisture seed, but the moisture clamp `[0,1]` means the wettest coasts saturate — moisture gains diminish toward `currentStrength`→2; temperature moderation is unaffected.) |
